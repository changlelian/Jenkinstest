...
INSTANTIATE_TEST_SUITE_P(Parameters, ProfilerParameterFilter, 
                                     ::testing::Values(std::make_pair("None", 0),
                                                       std::make_pair("Mean", 1),
                                                       std::make_pair("Median", 2)
                                                       ));
...
TEST_P(ProfilerParameterFilter, Filter) {
    ...
    testValue(gProPtr, profile_processing::Filter::name, modeMap);
}


INSTANTIATE_TEST_SUITE_P(Parameters, ProfilerParameterMeanFilterWindowSize, 
                                     ::testing::Values(std::make_pair("WindowSize_2", 0),
                                                       std::make_pair("WindowSize_4", 1),
                                                       std::make_pair("WindowSize_8", 2),
                                                       std::make_pair("WindowSize_16", 3),
                                                       std::make_pair("WindowSize_32", 4)
                                                       ));
// [DepthTriggerSource]: Testing the [set/getEnumValue] Interface with Normal Values
TEST_P(ProfilerParameterMeanFilterWindowSize, DepthTriggerSourceEnumValue) {
	...
	std::pair<std::string, int> modeMap = GetParam();
    std::cout << modeMap.first  << modeMap.second << std::endl;
    testValue(gProPtr, profile_processing::MeanFilterWindowSize::name, modeMap);
}



INSTANTIATE_TEST_SUITE_P(Parameters, ProfilerParameterMedianFilterWindowSize, 
                                     ::testing::Values(std::make_pair("WindowSize_3", 0),
                                                       std::make_pair("WindowSize_5", 1),
                                                       std::make_pair("WindowSize_7", 2),
                                                       std::make_pair("WindowSize_9", 3)
                                                       ));
// [DepthTriggerSource]: Testing the [set/getEnumValue] Interface with Normal Values
TEST_P(ProfilerParameterMedianFilterWindowSize, MedianFilterWindowSize) {
	...
	std::pair<std::string, int> modeMap = GetParam();
    std::cout << modeMap.first  << modeMap.second << std::endl;
    testValue(gProPtr, profile_processing::MedianFilterWindowSize::name, modeMap);
}
...
TEST_P(ProfilerParameterGapFilling, EncoderTriggerInterval) {
	...
	// by namespace
	testValue(gProPtr, profile_processing::GapFilling::name, setValue);
}