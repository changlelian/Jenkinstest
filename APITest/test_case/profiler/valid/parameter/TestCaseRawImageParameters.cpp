...
INSTANTIATE_TEST_SUITE_P(Parameters, ProfilerParameterExposureMode, 
                                     ::testing::Values(std::make_pair("HDR", 0),
                                                       std::make_pair("Timed", 1)
                                                       ));
...
TEST_P(ProfilerParameterExposureMode, ExposureMode) {
    ...
    testValue(gProPtr, brightness_settings::ExposureMode::name, modeMap);
}
...
TEST_P(ProfilerParameterExposureTime, ExposureTime) {
    ...
    case 1:
        ...
        for (int setvalue = 4; setvalue <=5200; setvalue+=4)
            testValue(gProPtr, brightness_settings::ExposureTime::name, setvalue);
        break;
...
TEST_P(ProfilerParameterHdrFirstThreshold, HdrFirstThreshold) {
	...
	// by namespace
	testValue(gProPtr, brightness_settings::HdrFirstThreshold::name, setValue);
}
...
TEST_P(ProfilerParameterHdrSecondThreshold, HdrSecondThreshold) {
	const double setValue = GetParam() * 0.100;
	// by namespace
	testValue(gProPtr, brightness_settings::HdrSecondThreshold::name, setValue);
}
...
INSTANTIATE_TEST_SUITE_P(Parameters, ProfilerParameterAnalogGain, 
                                     ::testing::Values(std::make_pair("Gain_1_0", 0),
                                                       std::make_pair("Gain_1_3", 1),
                                                       std::make_pair("Gain_1_9", 2),
                                                       std::make_pair("Gain_2_8", 3),
                                                       std::make_pair("Gain_5_5", 4)
                                                       ));
...
TEST_P(ProfilerParameterAnalogGain, AnalogGain) {
    ...
    switch (JudgeProfilerType(gProPtr))
    {
    case ProfilerType::LNX_8030:
        break;
    
    default:
        testValue(gProPtr, brightness_settings::AnalogGain::name, modeMap);
        break;
    }
...
INSTANTIATE_TEST_SUITE_P(Parameters, ProfilerParameterAnalogGainFor8030, 
                                     ::testing::Values(std::make_pair("Gain_1_0", 0),
                                                       std::make_pair("Gain_1_3", 1),
                                                       std::make_pair("Gain_2_0", 2),
                                                       std::make_pair("Gain_3_0", 3)
                                                       ));
...
TEST_P(ProfilerParameterAnalogGainFor8030, AnalogGainFor8030) {
    ...
    {
    case ProfilerType::LNX_8030:
        testValue(gProPtr, brightness_settings::AnalogGainFor8030::name, modeMap);
        break;
    default:
        break;
    }
...
TEST_P(ProfilerParameterDigitalGain, DigitalGain) {
	...
	// by namespace
	testValue(gProPtr, brightness_settings::DigitalGain::name, setValue);
}
...
TEST_P(ProfilerParameterLaserPower, LaserPower) {
	const int setValue = GetParam();
	// by namespace
	testValue(gProPtr, brightness_settings::LaserPower::name, setValue);
}


INSTANTIATE_TEST_SUITE_P(Parameters, ProfilerParameterZDirectionRoi, 
                                     ::testing::Values(std::make_pair("ImageHeight_1_1", 0),
                                                       std::make_pair("ImageHeight_1_2", 1),
                                                       std::make_pair("ImageHeight_1_4", 2),
                                                       std::make_pair("ImageHeight_1_8", 3),
                                                       std::make_pair("ImageHeight_1_16", 4)
                                                       ));
...
TEST_P(ProfilerParameterZDirectionRoi, ZDirectionRoi) {
	...
	std::pair<std::string, int> modeMap = GetParam();
    std::cout << modeMap.first  << modeMap.second << std::endl;
    testValue(gProPtr, roi::ZDirectionRoi::name, modeMap);
}
